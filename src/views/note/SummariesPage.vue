	...
	<div>
		...
		<a-table :columns="columns" :data-source="dataSource" :pagination="pagination" :loading="loading"
			...
			<template #expandedRowRender="{ record }">
				<p style="margin: 0" v-html="record.content.replace(/\n/g, '<br>') "></p>
			</template>
...
const fetchData = async (params = {}) => {
	...
	try {
		...
		const response = await apiClient.get(`u/notes/${noteId}/summaries`, {
			params: formatParams
		});

		dataSource.value = response.data.data.summaries || [];
